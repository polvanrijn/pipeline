...
PELL_man = create_cross_fold_template("PELL", cross_validation_by = "sentence", num_folds = 30, feature_sets = c("baseline", "both", "new"))
PELL_man$csv = '~/features/PELL/manually_combined_features.csv'
PELL_MAN_job = slurm_run(classify, PELL_man, jobname = "PELL_man")
PELL_man = get_result_as_df(PELL_MAN_job)
write.csv(PELL_man, paste0(Sys.getenv("PT_DIR"), "Results/03_classification/PELL_MAN_base_both_new.csv"), row.names = F)
